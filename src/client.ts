...
export default class PushReceiver extends EventEmitter {
  private readonly config: Types.ClientConfig;
  ...
  public connect = async (): Promise<void> => {
    ...
    if (this.config.credentials) {
      ...
      Logger.warn("Missing credentials... Running register.");
  ...
  private sendHeartbeatPing() {
    ...
    this.socket.write(Buffer.concat([Buffer.from([MCSProtoTag.kHeartbeatPingTag]), buffer]));
  }
  ...
  private sendHeartbeatPong(object) {
    ...
    this.socket.write(Buffer.concat([Buffer.from([MCSProtoTag.kHeartbeatAckTag]), buffer]));
  }
  ...
  private sendLogin() {
    ...
    this.socket.write(Buffer.concat([Buffer.from([Variables.kMCSVersion, MCSProtoTag.kLoginRequestTag]), buffer]));
  }
  ...
  private handleMessage = ({ tag, object }: Types.DataPacket): void => {
    ...
    switch (tag) {
      ...
      case MCSProtoTag.kIqStanzaTag:
        Logger.debug(
          "IqStanza: If anyone knows what is this and how to respond, please let me know! - message: ",
